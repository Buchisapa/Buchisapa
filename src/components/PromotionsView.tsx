...
import { PROMOTIONS, MenuItem, PromotionItem } from '../data/menuData';
...
export const PromotionsView: React.FC<PromotionsViewProps> = ({
  ...
  const getPromoMenuItem = (promo: PromotionItem): MenuItem => ({
    id: promo.id,
    name: promo.title,
  ...
  const handleQuickAdd = (e: React.MouseEvent, promo: PromotionItem) => {
    e.stopPropagation();
    const promoMenuItem = getPromoMenuItem(promo);
  ...
  const handleCardClick = (promo: PromotionItem) => {
    onSelectItem(getPromoMenuItem(promo));
  };
  ...
  return (
    <div className="bg-[#f8f9fa] min-h-[85vh] py-3 sm:py-5 px-3 sm:px-6 lg:px-8">
      <div className="max-w-3xl mx-auto">
        ...
        {/* List of Promotions Cards (Exact Pardos-style layout with full-size image) */}
              ...
              >
                ...
                <div className="p-3.5 sm:p-4.5 flex-1 flex flex-col justify-between min-w-0">
                  ...
                  <div className="flex items-center justify-between gap-2 mt-2 pt-1">
                    ...
                    <button
                      type="button"
                      onClick={(e) => handleQuickAdd(e, promo)}
